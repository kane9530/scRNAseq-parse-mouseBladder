...
seurat_colors <- ArchR::paletteDiscrete(names(manual_annotations), set="stallion")
```
...
unannotated_colors <- seurat_colors
names(unannotated_colors) <- levels(mbladder@meta.data$seurat_clusters)

# Plotting dimplots
seurat_dimplot <- DimPlot(mbladder, reduction = "umap", group.by="seurat_clusters", pt.size = 0.5,
        repel=TRUE, cols = unannotated_colors, label = TRUE)
annotated_dimplot_labels <- DimPlot(mbladder, reduction = "umap",  pt.size = 0.5,
        repel=TRUE, label=TRUE, label.box=TRUE, cols=seurat_colors) + NoLegend()
annotated_dimplot <- DimPlot(mbladder, reduction = "umap",  pt.size = 0.5,
        repel=TRUE, cols=seurat_colors) 
